...
if __name__ == "__main__":
    ...
    for i in range(K_Size):
        ...
        ensemble_cvfold_pred = 0

        for j in range(number_of_models):

            model = getTextCNNInstance()
            ...
            for step in range(num_steps):
                ...
                feed_dict = {model.input: x_batch, model.label: y_batch}

                _, loss_val = model.sess.run([model.optimize, model.loss], feed_dict=feed_dict)
                average_loss += loss_val

                if step % 100 == 0:
                    summary, loss_val, acc = model.sess.run([model.summary, model.loss, model.accuracy],
                                                            feed_dict=feed_dict)
            ...
            feed_dict = {model.input: test_x, model.label: test_y}
            accuracy = model.sess.run(model.accuracy, feed_dict=feed_dict)
            print("Accuracy", step, ': ', accuracy)

            # Calculate Confusion Matrix
            feed_dict = {model.input: test_x, model.label: test_y}
            predict = model.sess.run(model.predictions, feed_dict=feed_dict)

            ensemble_cvfold_pred += predict
            # print(ensemble_cvfold_pred)

        ensemble_total_pred.append(ensemble_cvfold_pred)

    ensemble_total_pred = np.squeeze(np.array(ensemble_total_pred)).reshape((-1))
    ensemble_total_pred = list(map(lambda x: 1 if x >= 2 else 0, ensemble_total_pred))
    print("final", ensemble_total_pred)
    label_Y = return_Y(filename=INDEXED_TOKENIZED_ASSESSMENT_RESULT_FILENAME)

    # Calculate Confusion Maxtrix

    for step, pred in enumerate(ensemble_total_pred):
        print(pred)
        if label_Y[step] == [0] and [pred] == [0]:
            FN += 1
        if label_Y[step] == [1] and [pred] == [0]:
            TN += 1
        if label_Y[step] == [1] and [pred] == [1]:
            TP += 1
        if label_Y[step] == [0] and [pred] == [1]:
            FP += 1
    ...
    with open('cnn.csv', 'w', encoding='utf-8', newline='') as f:
        wr = csv.writer(f)
        wr.writerow(ensemble_total_pred)
"""
