...
class TextCNN(object):
    ...
    def prepare_model(self,sequence_length, num_classes, word2vec, embedding_size, filter_sizes, num_filters, dropout_keep_prob, l2_reg_lambda=0.0):
        ...
        with tf.name_scope('output'):
            ...
            h_pool_flat = tf.matmul(h_drop, W)+b


            W2 = tf.get_variable('W2', shape=[2, num_filters_total],
                                initializer=tf.contrib.layers.xavier_initializer())
            b2 = tf.Variable(tf.constant(0.1, shape=[num_filters_total]), name='b2')

            h_pool_flat = tf.matmul(h_pool_flat, W2) + b2
            W3 = tf.get_variable('W3', shape=[num_filters_total, num_classes],
                                initializer=tf.contrib.layers.xavier_initializer())
            ...
            #train용 -drop out 적용
            scores = tf.nn.xw_plus_b(h_pool_flat, W3, b3, name='scores')

            #Accuracy 계산용 -drop out 미적용
            scores4Acc = tf.nn.xw_plus_b(h_pool_flat, W3, b3, name='scores4acc')
